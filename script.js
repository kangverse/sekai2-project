const MEDIA_V='?v=202608062330';   // bump when the video assets are re-encoded
const types=[
...
function renderAnnotationDemo(key){
  ...
  if(tabsEl)tabsEl.querySelectorAll('button').forEach(b=>{
    ...
    // the row scrolls sideways now, so a tab picked by keyboard can be off-screen
    if(on&&tabsEl.scrollWidth>tabsEl.clientWidth)b.scrollIntoView({block:'nearest',inline:'nearest'})});
...
fetch('assets/data/panoramic_cases.json?v=202608062330').then(r=>r.json()).then(data=>{panoramicCases=data;const motions=['All',...new Set(data.map(x=>x.motion))];reconstructionFilters.innerHTML=motions.map((x,i)=>`<button class="recon-filter ${i===0?'active':''}" data-motion="${x}">${x.replaceAll('-',' ')}</button>`).join('');renderPanoramicCases()});
...
fetch('assets/data/cases.json?v=202608062330').then(r=>r.json()).then(data=>{caseData=data;loadHomepageTrajectory('drone');loadPanoShowcase('panorama-serpentine')});
...
(function(){
  ...
  Promise.all([
    fetch('assets/data/world.svg?v=202608062330').then(r=>r.text()),
    fetch('assets/data/geo_countries.json?v=202608062330').then(r=>r.json())
...
(function(){
  ...
  fetch('assets/data/attributes.json?v=202608062330')
    .then(r=>r.json())
...
fetch('assets/data/annotation_cases.json?v=202608062330').then(r=>r.json())
