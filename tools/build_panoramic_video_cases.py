...
# ViPE pose rate: 60 fps sources are decimated to 30 fps, 30 fps sources kept as-is.
POSE_FPS = 30.0
SPECS = {
...
def render_preview(url, start, video_out, poster_out, seconds=15, fps=24):
    # fps was 2 here originally, which made the equirectangular previews look frozen
    # rather than merely soft. Keep this at a watchable rate.
    ...
    encoder.width, encoder.height, encoder.pix_fmt = 1280, 640, "yuv420p"
    ...
    for frame in source.decode(stream):
        ...
        small = frame.reformat(width=1280, height=640, format="yuv420p")
        small.pts = written
...
def pose_payload(path):
    data = np.load(path); pose = data["cam_c2w"] if "cam_c2w" in data else data["data"]
    pose = np.asarray(pose, float)
    finite = np.isfinite(pose).all(axis=(1, 2))
    # ViPE poses are at 30 fps (60 fps sources decimated to 30) and `inds` are
    # contiguous indices in that base, so the trajectory spans (last-first+1)/30.
    # The caption's last segment end time is NOT the trajectory duration.
    inds = np.asarray(data["inds"] if "inds" in data else np.arange(len(pose)))[finite]
    duration = float((inds[-1] - inds[0] + 1) / POSE_FPS)
    pose = pose[finite]
    ...
    return {"positions": np.round(xyz, 6).tolist(),
            ...
            "num_frames": len(pose), "duration": round(duration, 3),
            "fps": round(len(pose)/duration, 6), "pose_fps": POSE_FPS,
            "pose_frames": int(len(inds))}
...
def main():
    ...
    for key, (clip, start, title, motion) in SPECS.items():
        ...
        cases[key] = {"clip": clip, "dataset": "panoramic", "title": title,
                      ...
                      "pose3d": pose_payload(row["pose_path"]),
